...
/**
 * Contrôleur de la page 2 de l'application JavaFX.
 * Cette classe gère l'affichage des adolescents et de leur appariement,
 * ainsi que l'affichage de différents critères de compatibilité sous forme de sliders.
 * 
 * @version 1.0
 * @since 2025
 */
public class Page2Controller {
    ...
    /** Labels affichant les informations personnelles de l’adolescent sélectionné. */
    @FXML
    ...
    /** Sliders représentant les affinités ou compatibilités entre deux adolescents selon différents critères (âge, allergies, préférences alimentaires, etc). */
    @FXML
    private Slider birthDateSlider, guestAnimalAllergySlider, guestFoodConstraintSlider, hobbiesSlider, genderSlider;

    /** TableView affichant la liste des adolescents disponibles dans la plateforme. */
    @FXML
    private TableView tabAdolescents;

    /** Colonnes de la TableView représentant le prénom, le nom, la date de naissance et le pays des adolescents. */
    @FXML
    private TableColumn<ObservableList<String>, String> colFORENAME, colNAME, colBIRTH_DATE, colCOUNTRY;
    
    /** Plateforme contenant les données générales du séjour et les adolescents inscrits. */
    public PlateformeSejour plateformeSejour = null;

    /** Objet permettant de gérer les appariements entre adolescents. */
    public Affectation affectation = null;

    /** Tableau contenant les objets Adolescent avec leurs données complètes. */
    public Adolescent[] listAdolescents = null;

    /**
     * Initialise le contrôleur après le chargement du fichier FXML.
     * Configure les colonnes de la TableView et ajoute un listener pour détecter
     * les sélections de lignes afin d’afficher les détails de l’adolescent sélectionné.
     */
    @FXML
    ...
    /**
     * Initialise les données nécessaires au fonctionnement de la page,
     * notamment la liste des adolescents, la plateforme de séjour et l’affectation.
     * Met à jour la TableView avec les adolescents et sélectionne une ligne spécifique.
     *
     * @param ligne La ligne sélectionnée dans la table (informations d’un adolescent).
     * @param affectation L’objet de gestion des appariements entre adolescents.
     * @param listAdolescents Le tableau contenant tous les objets Adolescent.
     * @param plateformeSejour La plateforme contenant les données des séjours.
     */
    ...
    /**
     * Affiche les détails d’un adolescent sélectionné et, s’il existe,
     * les détails de son adolescent apparié.
     * Met également à jour les sliders pour refléter la compatibilité entre eux.
     *
     * @param ligne La ligne de données sélectionnée dans la TableView correspondant à un adolescent.
     */
     ...
     * Redirige vers la page d’accueil (Page1) tout en conservant les données actuelles.
     *
     * @throws IOException Si une erreur survient lors du chargement de la nouvelle page FXML.
     */
    @FXML
    ...
    /**
     * Gère l’événement de clic sur le bouton "Préférences".
     * Redirige vers la page de préférences (Page3) en passant les données nécessaires.
     *
     * @throws IOException Si une erreur survient lors du chargement de la nouvelle page FXML.
     */
    @FXML
